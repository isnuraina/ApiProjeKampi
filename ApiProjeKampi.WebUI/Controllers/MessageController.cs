...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> MessageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7230/api/Messages");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7230/api/Messages?id=" + id);
            return RedirectToAction("MessageList");
        }
       ...
       "https://localhost:7230/api/Messages/GetMessage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateMessage(UpdateMessageDto updateMessageDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7230/api/Messages/", stringContent);
            return RedirectToAction("MessageList");
        }
        ...
        public async Task<IActionResult> AnswerMessageWithOpenAI(int id, string prompt)
        {
            ...
            var requestData = new
            {
                ...
                messages = new[]
                {
                    ...
                    new {
                        role="system",
                        content="Sen bir restoran için kullanıcıların göndermiş oldukları mesajları detaylı ve olabildiğince olumlu," +
                        " müşteri memnunyeti gözeten cevaplar veren bir yapay zeka aracısın.Amacımız kullanıcı tarafından gönderilen" +
                        " mesajlara en olumlu ve mantıklı cevapları sunabilmek. "
                    },
                    new {
                        role="user",
                        content=prompt
        ...
        public async Task<IActionResult> SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(createMessageDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client2.PostAsync("https://localhost:7230/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("MessageList");
            }
